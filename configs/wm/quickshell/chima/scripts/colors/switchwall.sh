...
SHELL_CONFIG_FILE="$XDG_CONFIG_HOME/chima/config.json"
...
set_wallpaper_path() {
    local source_path="$1"
    local config_dir="$XDG_CONFIG_HOME/chima"

    # Get file extension
    local extension="${source_path##*.}"
    local dest_path="$config_dir/wallpaper.$extension"

    # Create config directory if it doesn't exist
    mkdir -p "$config_dir"

    # Copy wallpaper to dedicated location (this will replace if same extension exists)
    if [ -f "$source_path" ]; then
        cp "$source_path" "$dest_path"
        echo "Desktop wallpaper copied to: $dest_path"

        # After successful copy, remove old wallpaper files with different extensions to avoid orphans
        for old_file in "$config_dir"/wallpaper.*; do
            if [ -f "$old_file" ] && [ "$old_file" != "$dest_path" ]; then
                rm -f "$old_file"
            fi
        done
    else
        echo "Warning: Source wallpaper not found: $source_path"
        return 1
    fi

    # Update config to point to the copied file
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        # First set to empty to trigger a change event, then set to the new path
        jq '.background.wallpaperPath = ""' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
        sleep 0.05
        jq --arg path "$dest_path" '.background.wallpaperPath = $path' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
    fi
}
...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            # Copy and set wallpaper path (returns the copied path)
            set_wallpaper_path "$imgpath"

            # Use the copied video path for mpvpaper
            local config_dir="$XDG_CONFIG_HOME/chima"
            local extension="${imgpath##*.}"
            local video_path="$config_dir/wallpaper.$extension"
    ...
    source "$(eval echo $CHIMA_VIRTUAL_ENV)/bin/activate"
...
main() {
    ...
    # Check if imgpath exists when using --noswitch
    if [[ -n "$noswitch_flag" && -n "$imgpath" && ! -f "$imgpath" ]]; then
        echo "[switchwall.sh] Warning: Wallpaper file not found: $imgpath" >&2
        echo "[switchwall.sh] The config points to a missing file. Please set a new wallpaper." >&2
        # Try to find the hyprlock wallpaper as fallback
        local hyprlock_wallpaper="${XDG_CONFIG_HOME:-$HOME/.config}/hypr/hyprlock-wallpaper.png"
        if [[ -f "$hyprlock_wallpaper" ]]; then
            echo "[switchwall.sh] Using hyprlock wallpaper as fallback: $hyprlock_wallpaper" >&2
            imgpath="$hyprlock_wallpaper"
            noswitch_flag=""  # Treat as regular switch to copy the file
        else
            echo "[switchwall.sh] Error: No fallback wallpaper available. Exiting." >&2
            exit 1
        fi
    fi
