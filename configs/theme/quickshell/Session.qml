        ...
        sourceComponent: PanelWindow { // Session menu
            ...
            ColumnLayout { // Content column
                ...
                GridLayout {
                    ...
                    // Top row: Lock, Logout, Sleep, Hibernate
                    SessionActionButton {
                        id: sessionLock
                    ...
                    SessionActionButton {
                        ...
                        onClicked: { root.closeAllWindows(); Quickshell.execDetached(["bash", "-c", "hyprctl dispatch exit || loginctl terminate-session"]); sessionRoot.hide() }
                        onFocusChanged: { if (focus) sessionRoot.subtitle = buttonText }
                    ...
                    // Bottom row: Shutdown, Reboot, Reboot to Firmware Settings, Task Manager
                    SessionActionButton {
                        id: sessionShutdown
                        buttonIcon: "power_settings_new"
                        buttonText: Translation.tr("Shutdown")
                        onClicked:  { root.closeAllWindows(); Quickshell.execDetached(["bash", "-c", "systemctl poweroff || shutdown -h now"]); sessionRoot.hide() }
                        onFocusChanged: { if (focus) sessionRoot.subtitle = buttonText }
